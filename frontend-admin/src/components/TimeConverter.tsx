...
export const convertUTCToFormat = (
    utcTimeString: number | Date | string,
    format?: string,
    ...
    if (isNaN(date.getTime())) {
        return gLang('admin.timeInvalid');
    }
 ...
 * 例如："2小时前"、"3天前"、"刚刚"
 */
export const getRelativeTimeText = (utcTime: number | Date | string): string => {
    const date =
    ...
    if (diffSeconds < 60) {
        return diffSeconds <= 0
            ? gLang('admin.timeJustNow')
            : gLang('admin.timeSecondsAgo', { n: String(diffSeconds) });
    } else if (diffMinutes < 60) {
        return gLang('admin.timeMinutesAgo', { n: String(diffMinutes) });
    } else if (diffHours < 24) {
        return gLang('admin.timeHoursAgo', { n: String(diffHours) });
    } else if (diffDays < 30) {
        return gLang('admin.timeDaysAgo', { n: String(diffDays) });
    } else if (diffMonths < 12) {
        return gLang('admin.timeMonthsAgo', { n: String(diffMonths) });
    } else {
        return gLang('admin.timeYearsAgo', { n: String(diffYears) });
    }
};
